...
def evaluate_and_plot_trip(fpath: str, trip: np.ndarray, model, meta, args, sample_idx: int) -> Dict[str, Any]:
    ...
    # ---- INDIVIDUAL STATIC PLOTTING (Only if --no_plots is NOT set) ----
...
def main():
    ...
    # Buffer to hold all lines if we are making a single combined picture OR folium map
    combined_plot_data = [] 
    ...
    if args.same_pic and combined_plot_data:
        ...
        # 3. Add Professional Map Features
        land_feature = cfeature.GSHHSFeature(scale='h', levels=[1], 
                                           facecolor=STYLE["land"], 
                                           edgecolor=STYLE["edge"],
                                           linewidth=0.5)
        ...
        # Title with Metrics
        title_str = f"Combined Trajectories (N={n_plots})\nMean ADE: {mean_ade:.2f} km | Mean FDE: {mean_fde:.2f} km"
