...
def evaluate_and_plot_trip(fpath: str, trip: np.ndarray, model, meta, args, sample_idx: int) -> Dict[str, Any]:
    ...
    # ---- PLOTTING WITH MANUAL BACKGROUND CONTROL ----
    fname_png = "skipped"
    if not args.no_plots:
        ...
        # 1. Determine Plot Bounds (Extent)
        # If user provided manual plot bounds, use them.
        # Otherwise, calculate auto bounds from the trajectory data.
        if args.plot_lat_min is not None and args.plot_lat_max is not None:
            # Use Manual Map Background
        ...
        # Apply the Map Background Bounds
...
def main():
    ...
    # Model/Data Bounds (Changing these moves the LINES)
    ...
    # Plot Viewport Bounds (Changing these moves the CAMERA only)
    p.add_argument("--plot_lat_min", type=float, default=None, help="Manual Plot Bottom Edge")
    p.add_argument("--plot_lat_max", type=float, default=None, help="Manual Plot Top Edge")
    p.add_argument("--plot_lon_min", type=float, default=None, help="Manual Plot Left Edge")
    p.add_argument("--plot_lon_max", type=float, default=None, help="Manual Plot Right Edge")
    ...
    print("-" * 40)
    print(f"[DEBUG] DATA BOUNDS (Used for Line Calculation):")
    print(f"   Lat: {args.lat_min} to {args.lat_max}")
    print(f"   Lon: {args.lon_min} to {args.lon_max}")
    print("   * If ships are on land, THESE are the numbers that are wrong.")
    print("-" * 40)
    ...
    if metrics:
        ...
        with open(out_csv, "w", newline="") as fp:
            ...
            w.writeheader()
            w.writerows(metrics)
        print(f"Mean ADE: {np.mean([m['ade_km'] for m in metrics]):.2f} km")
