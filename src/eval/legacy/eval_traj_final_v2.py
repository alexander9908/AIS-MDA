...
try:
    ...
    print("[Warning] Could not import TrAISformer, falling back...")
    from src.models.traisformer_old import TrAISformer, BinSpec
try:
    from src.models.tptrans_transformer import TPTrans
except ImportError:
    print("[Warning] Could not import TPTrans V3, falling back...")
    from src.models.tptrans_gru import TPTrans # Fallback GRU version
...
def compute_horizon_metrics(lats_true, lons_true, lats_pred, lons_pred, 
    ...
    """
    Computes ADE and FDE at specific time steps (e.g. 1h=12 steps).
    ...
    for h in horizon_steps:
        # ADE Key (Average error up to horizon)
        key_ade = f"ade_{h//12}h_km"
        # FDE Key (Error AT horizon)
        key_fde = f"fde_{h//12}h_km"
        
        if n_min > h:
            # Mean error from step 0 to step h (inclusive)
            errors = [haversine_km(lats_true[i], lons_true[i], lats_pred[i], lons_pred[i]) 
                     for i in range(h + 1)]
            metrics[key_ade] = float(np.mean(errors))
            metrics[key_fde] = float(errors[-1]) # Error at exact step h
        else:
...
def evaluate_trip_wrapper(fpath: str, trip: np.ndarray, model, meta, args, sample_idx: int) -> Dict[str, Any]:
    ...
    # Branch A: Kalman (With land stop)
    if args.model.lower() == "kalman":
        ...
        raw_kf_lat = eval_res["pred_real"][:, 0]
        raw_kf_lon = eval_res["pred_real"][:, 1]
        
        # Land Check: Stop if hits land
        valid_lats, valid_lons = [], []
        curr_l, curr_o = cur_lat, cur_lon
        for k in range(len(raw_kf_lat)):
            cand_l, cand_o = float(raw_kf_lat[k]), float(raw_kf_lon[k])
            safe, _ = is_path_safe(curr_l, curr_o, cand_l, cand_o)
            if safe:
                valid_lats.append(cand_l)
                valid_lons.append(cand_o)
                curr_l, curr_o = cand_l, cand_o
            else:
                # Hits land -> STOP prediction here
                break 
        
        pred_lats_deg = np.array(valid_lats)
        pred_lons_deg = np.array(valid_lons)

    # Branch B: TPTrans (Regression + RayCast)
        ...
        while steps_generated < N_future:
            ...
            X_tensor = torch.from_numpy(curr_seq_in[-Tin:, :][None, ...]).to(device)
        ...
        pred_lats_deg = np.array(res_lats[1:]) 
    ...
    # Branch C: TrAISformer (Discrete Autoregressive)
        ...
        ### Land Correction: Project each point to water (original)#####
        ...
        ### Land Check: Stop if hits land ##### Kalman Style
        # valid_lats, valid_lons = [], []
        # curr_l, curr_o = cur_lat, cur_lon
        # for k in range(len(raw_lats)):
        #     cand_l, cand_o = float(raw_lats[k]), float(raw_lons[k])
    ...
    res = {
        ...
        "trip_len_h": round(len(lats_true) * 5 / 60, 2), # Approx length
        **h_metrics,
        "plot_data": None,
        "raw_data": None
    }
...
def do_individual_plot(mmsi, tid, d, args, idx):
    ...
    ax.legend()
    # Added FDE to title
    ax.set_title(f"MMSI {mmsi} | ADE {d['ade']:.2f} km | FDE {d['fde']:.2f} km")
...
def generate_folium_map(plot_data_list, out_dir, args):
    ...
    for d in plot_data_list: 
        # Added trip len to tooltip
        est_hours = (len(d['lats_true']) * 5.0) / 60.0
        tooltip_txt = f"MMSI: {d['mmsi']} | ADE: {d['ade']:.2f} km | FDE: {d['fde']:.2f} km | Len: {est_hours:.1f}h"
...
def main():
    ...
    if metrics:
        ...
        print("-" * 40)
        print(f"Mean ADE: {mean_ade:.3f} km | Median: {median_ade:.3f}")
        print(f"Mean FDE: {mean_fde:.3f} km | Median: {median_fde:.3f}")
        
        # Horizon stats (Printing FDE as requested)
        for h_idx, h_key in enumerate(['ade_1h_km', 'ade_2h_km', 'ade_3h_km']):
            fde_key = h_key.replace("ade", "fde")
            
            # ADE Stats
            ade_vals = [m[h_key] for m in clean_metrics if m.get(h_key) is not None]
            if ade_vals:
                print(f"{h_key}: Mean {np.mean(ade_vals):.3f} | Median {np.median(ade_vals):.3f} (N={len(ade_vals)})")
            
            # FDE Stats (Requested polish)
            fde_vals = [m[fde_key] for m in clean_metrics if m.get(fde_key) is not None]
            if fde_vals:
                print(f"{fde_key}: Mean {np.mean(fde_vals):.3f} | Median {np.median(fde_vals):.3f} (N={len(fde_vals)})")
