...
def csv2pkl(lon_min=LON_MIN, lon_max=LON_MAX,
    ...
    results = {file_name: {"total_messages": 0,
                           "filtered_messages": 0,
                           "cargo_tanker_messages": 0} for file_name in l_csv_filename}
    ...
    for csv_filename in logger.tqdm(l_csv_filename, desc=f'Reading csvs'):
        try:
            ...
            if cargo_tankers_only: # Keep only Cargo and Tanker ship types
                total_messages = lf.select(pl.len()).collect()[0,0]
                results[csv_filename]["filtered_messages"] = total_messages
                lf = lf.filter(pl.col("Ship type").is_in(["Cargo", "Tanker"]))
            ...
            if cargo_tankers_only:
                results[csv_filename]["cargo_tanker_messages"] = df.height
            else:
    ...
    if cargo_tankers_only:
        logger.log_summary({
            "total_cargo_tanker_messages": sum(info["cargo_tanker_messages"] for info in results.values())
        })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cargo_tankers_only", action='store_true', help="If set, only keep Cargo and Tanker ship types.")
    ...
    csv2pkl(input_dir=args.input_dir,
            ...
            cargo_tankers_only=args.cargo_tankers_only,
            logger=logger)