...
class AISDatasetBase(Dataset):
    """
    Base class for loading AIS trajectories from .pkl files.
    
    Features:
    1. Loads preprocessed trajectories (Lat, Lon, SOG, COG, etc.)
    2. specific 'start_mode' (e.g., 'kmeans') to sample difficult starting positions.
    3. Supports 'epoch_samples' to oversample files per epoch (for larger effective batch sizes).
    """
    def __init__(self, data_dir, max_seqlen=96, file_extension=".pkl",
        ...
        # Random Number Generators (Independent of global seed for reproducibility)
        seed = self.kmeans_config.get("random_state", 42)
        self._kmeans_rng = np.random.default_rng(seed)
        self._start_rng = np.random.default_rng(seed)
        
        self._prepare_metadata()

    def __len__(self): 
        # epoch_samples allows seeing the same file multiple times with different start offsets
        return len(self.file_list) * self.epoch_samples

    def _load_file(self, path):
        # Robust loader handling Pickle vs Joblib
        if self.joblib_used:
    ...
    def _prepare_metadata(self):
        """Scans all files to build indices and fit KMeans if requested."""
        ...
        # 1. Load all lengths and collect samples for KMeans
        for fname in self.file_list:
            ...
            try:
                ...
                if self.start_mode == "kmeans":
                    lat_lon = traj[:, :2]
                    # Subsample to save memory during fitting
                ...
                temp_trajs.append(np.zeros((0,4)))

        # 2. Fit KMeans if enabled
        if self.start_mode == "kmeans" and coords_samples:
            ...
            self._kmeans_model = KMeans(n_clusters=n_clusters, n_init=n_init, random_state=42)
            self._kmeans_model.fit(coords)
            self._centroids = self._kmeans_model.cluster_centers_

        # 3. Build lookup table for start indices
        ...
        for i, traj in enumerate(temp_trajs):
            ...
            # If KMeans, assign every valid start index to a cluster
    ...
    def __getitem__(self, idx):
        ...
        # Sanity clip for normalized data
        slice_traj[slice_traj > 0.9999] = 0.9999
        slice_traj[slice_traj < 0.0] = 0.0
        ...
        # Column 7 is usually timestamp in these preprocessed files
        time_start = int(traj[start_idx, 7]) if traj.shape[1] > 7 else 0
    ...
    def _pick_start_index_fast(self, file_idx):
        ...
        # Strategy: Pick a random cluster, then pick a start index from that cluster
...
class AISDataset(AISDatasetBase):
    """
    Unified Dataset for TPTrans and TrAISformer.
    Splits the loaded sequence into Past (X) and Future (Y).
    """
    def __init__(self, data_dir, window=64, horizon=12, output_features=2, 
        ...
        self._output_features = output_features # 2 for TPTrans, 4 for TrAISformer
        self._valid_indices = []
        
        # Filter indices: ensure the underlying file is long enough for window+horizon
    ...
    def __getitem__(self, idx):
        # Map virtual index to actual file index
        orig_idx = self._valid_indices[idx]
        
        # Get raw sequence from Base
        seq, mask, seqlen, mmsi, time_start = super().__getitem__(orig_idx)
        
        # Slice X (Past) and Y (Future)
        # seq shape is [max_seqlen, 4] -> (Lat, Lon, SOG, COG)
        X = seq[:self._window]
        
        # Slice Y based on required features
        # TPTrans (Regression) -> needs [horizon, 2] (Lat, Lon)
        # TrAISformer (Classification) -> needs [horizon, 4] (Lat, Lon, SOG, COG)
